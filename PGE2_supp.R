...
# Input files
curFilename = "PGE_organoid"
curTitle = "Pge2 vs Control"
...
## Plot clusters on UMAP embedding
{
  ...
  ggplot(dimred, aes(x = UMAP_1, y = UMAP_2, fill = as.factor(Cluster), color = as.factor(Cluster))) + geom_point() + theme_cem + geom_polygon(data = hulls, aes(x = x, y = y, fill = as.factor(Cluster), col = as.factor(Cluster)), alpha = 0.3) + geom_text(data = clusterMedian, aes(label = Cluster, group = as.factor(Cluster)), size = 5, color = "Black") + theme(legend.position = "none")  + scale_fill_manual(values = iwanthue(numClust)) + scale_color_manual(values = iwanthue(numClust))
  ggsave(paste0(curFilename, "_dimred_cluster_hull.png"), width = dimredPlotWidth, height = dimredPlotHeight)
  ggsave(paste0(curFilename, "_dimred_cluster_hull.pdf"), width = dimredPlotWidth, height = dimredPlotHeight)
...
# Pathways
{
  ...
  for(curPathway in unique(pathwayScoresZscore$Pathway)){	
    ...
    ggplot(sm3W) + geom_tile(aes(x = x, y = y, fill = Diff)) + theme_cem + 
      scale_fill_gradientn(name = "Pge2 vs Control difference", colors = pathwayColorsDiff, limits = c(-rangeDiff, rangeDiff)) + 
...
{	
  ...
  ggsave(file= 'PGE_Pseudotime_umap_rainbow.pdf', width=2.5, height=2.5, units="in")
  ...
  curr <- c('Control', 'PGE2')
  new <- c('Control', 'Pge2')
  celltype_sample_norm2$Type <- plyr::mapvalues(x = celltype_sample_norm2$Type, from = curr, to = new)
  ...
  ggplot(celltype_sample_norm3, aes(x = CellType, y = Proportion, fill = Type)) + geom_bar(stat="identity", position="dodge") + geom_errorbar(aes(ymin=Proportion-se, ymax=Proportion+se),position=position_dodge(width = 0.85),width=0.3) + theme_cem + scale_fill_manual(values = colorsType) + expand_limits(y = c(0)) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), panel.background = element_rect(fill = "white", colour = "Black")) + xlab("Cell Type") + ylab("Fraction of Cells") + labs(fill = "Sample") + scale_y_continuous(expand = expansion(mult = c(0, .1)))
  ggsave(paste0(curFilename, "_CellType_Bar_Type_BySample.png"), width = 10, height = 7)
...
## Plot gene expression on dimred embedding
{
  ...
  for(curGene in geneList)
  {
    ...
    sm3W = dcast(sm3, x+y~type, value.var="score")
    sm3W$AA_vs_Control = sm3W$Pge2 - sm3W$Control
...
for(i in cell_types){
  ...
  dimreda <- dimredaub %>% filter(Type == 'Pge2')
  ...
  ggplot(plot11, aes(x, y, fill=y>=0)) + geom_col(position = 'dodge', width = .0978619) + theme_cem + scale_fill_manual(values = c('#1b9e77', '#d95f02')) + theme(legend.position = "none") + ylab('Density') + xlab('Pseudotime') + geom_vline(data = quart, aes(xintercept = var2), size = .075) + geom_text(data = quart, aes(var1 , maxi, label = p_asterik), position=position_dodge2(0.2), size = 1.5, vjust=1, inherit.aes = FALSE) + theme(text = element_text(size=1),axis.text = element_text(size = 2) ,plot.title = element_text(size = 6), axis.text.x = element_text(size = 6), axis.text.y = element_text(size = 6), axis.title.x = element_blank(), axis.title.y = element_blank(), plot.margin = unit(c(.1, 0, 0, 0), "cm"))
  
  ggsave(paste0("AA_pseudo_density_", cell, ".pdf"), width = 2.2, height = 1, units="in")	
}
...

{
  ...
  ggsave(paste0("AA_pseudo_density_Stem2.pdf"), width = 2.2, height = 1, units="in")	
  ggsave(paste0("pge_pseudo_density_all.pdf"), width = 2.2, height = 1, units="in")	
  ...
  for(caseType in unique(dimred$Type))
  {
    for(ctrlType in unique(dimred$Type))
    {
      ...
      ggplot(diff_CaseVsCtrlM, aes(x = UMAP_1, y = UMAP_2)) +
        ggrastr::rasterise(geom_tile(aes_string(fill = caseVsCtrlName), alpha = 1), dpi = 50) + #use hannah's raster code here GGrastr
...
## Trajectory analysis
{
  ...
  #seuratExpSelAA = seuratExpSel[, colnames(seuratExpSel) %in% dimred$Cell[ dimred$Type == "Pge2"] ]
  ...
  seuratExpSelMedSumW_Case = as.matrix(setRemoveRownames(dcast(seuratExpSelMedSum2[ seuratExpSelMedSum2$Type == "Pge2", ], Gene~PseudotimeFactor, value.var="Expression")))
